...
PyRepObject *InventoryItem::GetInventoryRowset(EVEItemFlags _flag) const {
	util_Rowset rowset;
...
InventoryItem *InventoryItem::Split(InventoryItem *to_split, sint32 qty_to_take, bool notify) {
	...
	InventoryItem *res;
	//this is a bit dirty, but seems to be needed.
	if(notify) {
		res = Spawn(
			m_typeID, qty_to_take, 1, m_flag	//temp owner to cause the spawn via update
			);
		res->ChangeOwner(m_ownerID, notify);
	} else {
		res = Spawn(
			m_typeID, qty_to_take, m_ownerID, m_flag
			);
	}
	return( res );
}
...
//TODO: move this function somewhere, it dosent belong in InventoryItem::
int InventoryItem::GetSkillLevel(const uint32 typeID)  {
	if(!LoadContents())	// make sure skills are loaded
		return(0);
	std::map<uint32, InventoryItem *>::iterator cur, end;
	cur = m_contents.begin();
	end = m_contents.end();
	for(; cur != end; cur++) {
		if( typeID == cur->second->typeID() &&
		(cur->second->flag() == flagSkill || cur->second->flag() == flagSkillInTraining) )
			return(cur->second->skillLevel());
	}
	return(0);
}
...
bool InventoryItem::StackContainedItems(EVEItemFlags locFlag)
{
	...
	{
		i = cur->second;
		//Check if its in the flagged location
		if( i->flag() == locFlag )
		{
