...
bool ReprocessingDB::GetRecoverables(const uint32 typeID, std::vector<Recoverable> &into) {
	...
	if(!m_db->RunQuery(res,
				"SELECT mineralTypeID, amountPerBatch FROM invReprocessing WHERE itemTypeID=%lu" 
				" UNION "
				"SELECT mineralTypeID, amountPerBatch FROM invOreReprocessing WHERE oreTypeID=%lu",
				typeID,
				typeID
				))
	{
